    ...
    <div className="flex items-start gap-8">
      ...
      <div className="flex-1 min-w-0 pt-2">
        ...
        {(birthYear || passingYear) && (
          <p className="mt-1 text-body-2 text-r-muted">{birthYear && passingYear ? `${birthYear} – ${passingYear}` : birthYear || passingYear}</p>
        )}
        {(memorial?.brief_biography || memorial?.short_description) && (
          <p className="mt-2 max-w-md text-body-2 text-r-secondary" style={{ lineHeight: 1.6 }}>{memorial.brief_biography || memorial.short_description}</p>
        )}
      </div>
    </div>
  );
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <p className="text-body-2 font-medium text-r-text">No story generated yet</p>
        <p className="mt-1 text-body-2 text-r-muted">The story will appear here once the memorial has been generated.</p>
      </div>
    );
  }
  return <div className="flex flex-col items-center justify-center py-24"><p className="text-body-2 text-r-muted">Story tab — built by Sungjun</p></div>;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4" onClick={onClose}>
      <div className="relative max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
        <div className="aspect-[4/3] w-full overflow-hidden rounded-2xl bg-neutral-800">
          {photo.url ? <img src={photo.url} alt={photo.caption || ''} className="h-full w-full object-cover" /> : <div className="h-full w-full bg-neutral-700 flex items-center justify-center"><span className="text-neutral-500 text-sm">No image</span></div>}
        </div>
...
function AllPhotosTab({ albums }) {
          ...
          <button key={i} onClick={() => setOpenAlbum(album)} className="group text-left">
            <div className="aspect-square overflow-hidden rounded-2xl relative bg-r-card">
              {album.cover_photo_url || album.photos?.[0]?.url
                ? <img src={album.cover_photo_url || album.photos[0].url} alt={album.album_name} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
                : <div className="h-full w-full flex items-center justify-center bg-r-card"><svg width="28" height="28" fill="none" stroke="var(--color-r-muted)" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div>}
              <div className="absolute bottom-2 right-2 rounded-full bg-black/50 px-2 py-0.5 text-xs text-white backdrop-blur-sm">{album.photos?.length || 0}</div>
            </div>
  ...
  return (
    <div>
      <button onClick={() => setOpenAlbum(null)} className="mb-6 flex items-center gap-1.5 transition-colors text-body-2 text-r-muted bg-none border-none cursor-pointer"
