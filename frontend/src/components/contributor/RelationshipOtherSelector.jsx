'use client';
// frontend/src/components/contributor/RelationshipOtherSelector.jsx
...
export default function RelationshipOtherSelector({ inviteToken }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (localDraft.relationship_type !== CONTRIBUTOR_RELATIONSHIP_OTHER) {
        router.replace(`/contribute/${inviteToken}/relationship`);
        return;
      }
      if (isMounted) {
        setLabel(localDraft.relationship_label || "");
        setIsLoading(false);
      }
    }
  ...
  async function handleContinue() {
    ...
    try {
      await saveContributorRelationship(inviteToken, {
        relationshipType: CONTRIBUTOR_RELATIONSHIP_OTHER,
        relationshipLabel: trimmed,
      });
      ...
      <div className="flex-1 px-6 sm:px-[50px] pt-6 pb-16">
        <div className="page-shell">
          ...
          <div className="flex flex-col gap-3">
            <label htmlFor="relationship-other-label" className="text-2xl font-display font-normal text-r-text">
              Relationship type
            </label>
            <input
              ...
              style={{ border: error ? '1px solid var(--color-r-danger)' : '1px solid var(--color-r-border)' }}
              onFocus={(e) => { e.target.style.borderColor = 'var(--color-r-border-focus)'; }}
              onBlur={(e) => { e.target.style.borderColor = error ? 'var(--color-r-danger)' : 'var(--color-r-border)'; }}
            />
            {error ? (
              <p className="text-caption text-r-danger" role="alert">{error}</p>
            ) : null}
          </div>
