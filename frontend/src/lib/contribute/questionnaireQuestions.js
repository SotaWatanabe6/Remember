...
// "Question Design by Relationship Type" spec (NS-14). Each set is written
// from the contributor's side of the relationship (e.g. "parent" = the
// contributor is [first name]'s parent) and has 6 questions using
// [first name] as a placeholder, substituted with the subject's actual
// first name via formatQuestionPrompt.

export const RELATIONSHIP_QUESTION_SETS = {
  friend: [
 ...
 * to 'other' if the type isn't recognized. Accepts values like "Spouse",
 ...
 * Falls back to the raw prompt (with the placeholder still in it) if no
 * subject name is available yet.
 */
