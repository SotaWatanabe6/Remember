...
async function runPipelines(memorialId, jobId) {
  ...
  try {
    ...
    await updateJob(jobId, 70, 'Composing the biographical slideshow...')
...
router.post('/memorials/:id/generate', authMiddleware, async (req, res) => {
  try {
    ...
    if (memorial.status === 'complete') {
      return res.status(400).json({
        error: 'This memorial has already been generated. Generations are final and cannot be run again.',
      })
    }
    ...
    const { count: photoCount, error: photoCountError } = await supabase
      ...
      .eq('memorial_id', req.params.id)
    if (photoCountError) {
      return res.status(400).json({ error: photoCountError.message })
    }
    if (!photoCount || photoCount < 1) {
      return res.status(400).json({
        error: 'At least one contributor photo is required before generating the memorial.',
      })
    }
